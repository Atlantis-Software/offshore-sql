  ...
  if (select.options.skip || select.options.limit) {
    ...
    if (select.options.sort) {
      ...
      _.keys(select.options.sort).forEach(function (toSort) {
        ...
        if (sort !== '') {
          sort += connection.client.raw(', ?? ' + direction, [toSort]).toString();
        } else {
          sort = connection.client.raw('order by ?? ' + direction, [toSort]).toString();
        }
      });
      if (_.isUndefined(select.options.sort[select.pk])) {
        sort += connection.client.raw(', ??', [select.alias + '.' + select.pk]).toString();
      }
