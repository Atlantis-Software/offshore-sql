...
OracleDialect.prototype.joinId = function(select, relations) {
  if (_.keys(relations).length) {
    var joinSubquery = 'select 1 as "id" from dual';
    for (var i = 2; i <= _.keys(relations).length; i++) {
      joinSubquery += ' UNION ALL SELECT ' + i + ' from dual';
    }
    select.query.join(select.connection.client.raw('(' + joinSubquery + ') "joins" on 1 = 1'));
  }
};
...
OracleDialect.prototype.selectSkipLimit = function(connection, select) {
  ...
  if (select.options.skip || select.options.limit) {
    ...
    if (!select.options.sort) {
      ...
      var opts = _.cloneDeep(select.options);
      delete opts.skip;
      delete opts.limit;
      if (opts.joins) {
        opts.joins.forEach(function(join) {
          join.select = [];
        });
      }
      opts.select = [];
      ...
      var SklmtJoinSub = this.select(connection, collection, opts);
      SklmtJoinSub.query.select(select.alias + '.' + select.pk);
      SklmtJoinSub.query.groupBy(select.alias + '.' + select.pk);

      _.keys(sort).forEach(function(toSort) {
        var order = sort[toSort];
        if (!toSort.includes('.')) {
          toSort = select.alias + '.' + toSort;
        }
		    if (toSort !== select.alias + '.' + select.pk) {
          var alias = self.createAlias('SORT_', select.alias + '_' + toSort);
          if (order === 1) {
            SklmtJoinSub.query.min(toSort + ' as ' + alias);
            SklmtJoinSub.query.orderBy(alias, 'asc');
          } else {
            SklmtJoinSub.query.max(toSort + ' as ' + alias);
            SklmtJoinSub.query.orderBy(alias, 'desc');
          }
        } else {
          if (order === 1) {
            SklmtJoinSub.query.orderBy(toSort, 'asc');
          } else {
            SklmtJoinSub.query.orderBy(toSort, 'desc');
          }
        }
      });

      select.associations.forEach(function(association) {
        if (association.join.criteria.sort) {
          _.keys(association.join.criteria.sort).forEach(function(toSort) {
            var order = association.join.criteria.sort[toSort];
            var alias = self.createAlias('SORT_', association.join.alias + '_' + toSort);
            if (order === 1) {
              SklmtJoinSub.query.min(association.join.alias + '.' + toSort + ' as ' + alias);
              SklmtJoinSub.query.orderBy(alias, 'asc');
            } else {
              SklmtJoinSub.query.max(association.join.alias + '.' + toSort + ' as ' + alias);
              SklmtJoinSub.query.orderBy(alias, 'desc');
            }
          });
        }
      ...
      var SklmtJoin = connection.client.select(connection.client.raw('rownum "row_PARENT"'));
      SklmtJoin.select('SJB.' + select.pk);
      SklmtJoin.from(SklmtJoinSub.query.as('SJB'));

      select.query.leftJoin(SklmtJoin.as('SJ'), connection.client.raw('??.?? = ??.??', [select.alias, select.pk, 'SJ', select.pk]));
      select.query.select('SJ.row_PARENT');
    }
