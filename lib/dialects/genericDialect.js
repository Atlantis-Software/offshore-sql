...
GenericDialect.prototype.select = function (connection, collection, opts) {
  ...
  return select.query.then(function(res){console.log('res',res);return res;});
};
      ...
      skLmtQuery.andWhere(function(){
        for (var i in keys) {
          this.orWhere(function(){
            j = 0;
            while(j < i) {
              this.andWhereRaw( '?? = ??', [ skLmtAlias + '.' + keys[j], select.alias + '.' + keys[j] ] );
              j++;
            }
            var key = keys[i];
            if (select.options.sort[key]) {
              this.andWhereRaw( '?? > ??', [ select.alias + '.' + keys[i], skLmtAlias + '.' + keys[i] ] );
            }
            else {
              this.andWhereRaw( '?? < ??', [ select.alias + '.' + keys[i], skLmtAlias + '.' + keys[i] ] );
            }
