...
GenericDialect.prototype.parseType = function(type, value) {
  switch (type) {
    case 'integer':
      if (_.isString(value)) {
...
GenericDialect.prototype.count = function(connection, collection, opts) {
  ...
  var query = new CriteriaProcessor(connection, tableName, options, connection.client(tableName)).getQuery();
  query.count('* as cnt');
...
GenericDialect.prototype.select = function(connection, collection, opts) {
  ...
  return select.query.then(
    function(res){

      res.forEach(function(result){
        for (var key in result) {
          if (definition[key]) {
            result[key] = self.parseType(definition[key].type, result[key]);
          }
        }
      });
      return res;
...
GenericDialect.prototype.selectSkipLimit = function(connection,select) {
  if (select.options.skip || select.options.limit) {
    if (!select.options.joins) {
      ...
      var sklmtquery = new CriteriaProcessor(connection, 'SKLMT', select.options, connection.client(select.tableName + ' as SKLMT')).getQuery();
      sklmtquery.select('SKLMT.' + select.pk);
      if (select.options.skip) {
        sklmtquery.offset(select.options.skip);
      }
      if (select.options.limit) {
        sklmtquery.limit(select.options.limit);
      }
      select.query.andWhere(select.alias + '.' + select.pk, 'IN', connection.client.select('*').from(function() {
        ...
        new CriteriaProcessor(connection, select.tableName, select.options, this).getQuery();
        if (select.options.skip) {
