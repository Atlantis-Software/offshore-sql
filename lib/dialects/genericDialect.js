...
GenericDialect.prototype.select = function (connection, collection, opts) {
  ...
  if (select.options.skip || select.options.limit) {
    if (!select.options.joins) {
      ...
      //skLmtQuery.andWhere(Knex.raw(skLmtAlias + '.' + select.pk + ' >= ' + select.alias + '.' + select.pk));
...
GenericDialect.prototype.joinSkipLimit = function (connection, select, join) {
  ...
  if (join.select === false) {
    ...
    var skLmtAlias = this.createAlias('_SKLMT_', join.child);
    var skLmtQuery = connection.client(join.child + ' as ' + skLmtAlias).count('* as ' + skLmtAlias);
    ...
    skLmtQuery.andWhere(function () {
      for (var i in keys) {
        this.orWhere(function () {
          j = 0;
          while (j < i) {
            this.andWhere(Knex.raw(skLmtAlias + '.' + keys[j] + ' = ' + join.alias + '.' + keys[j]));
          }
          var key = keys[i];
          if (join.criteria.sort[key]) {
            this.andWhere(Knex.raw(skLmtAlias + '.' + keys[i] + ' > ' + join.alias + '.' + keys[i]));
          }
          else {
            this.andWhere(Knex.raw(skLmtAlias + '.' + keys[i] + ' < ' + join.alias + '.' + keys[i]));
          }
    ...
    //skLmtQuery.andWhere(Knex.raw(skLmtAlias + '.' + join.childKey + ' = ' + join.alias + '.' + join.childKey));
    skLmtQuery.groupBy(select.alias + '.' + select.pk);
    //skLmtQuery.andWhere(Knex.raw(skLmtAlias + '.' + join.childKey + ' = ' + join.alias + '.' + join.childKey));
    //skLmtQuery.andWhere(Knex.raw(skLmtAlias + '.' + join.parentKey + ' = ' + select.tableName + '.' + select.pk));
    //skLmtQuery.andWhere(Knex.raw(skLmtAlias + '.' + join.childKey + ' = ' + join.alias + '.' + join.childKey));
    select.selection.push(skLmtQuery.as(skLmtAlias));
    if (join.criteria.skip) {
      select.query.andHaving(skLmtAlias, '>=', join.criteria.skip);
    }
    if (join.criteria.limit) {
