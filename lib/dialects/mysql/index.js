...
MysqlDialect.prototype.joinSkipLimit = function(connection, select, join) {
  ...
  if (join.select === false) {
    ...
    select.query = connection.client.queryBuilder().select('*').from(select.query);
...
MysqlDialect.prototype._getDenseRank = function(connection, query, partitionBy) {
  ...
  return connection.client.queryBuilder().select('*', connection.client.raw(incDenseRank + updateVar)).from(query.as('selectQuery')).join(connection.client.raw(initVar)).as('denseRankQuery');
};
