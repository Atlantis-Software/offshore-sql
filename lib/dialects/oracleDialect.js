...
OracleDialect.prototype.selectSkipLimit = function(connection, select) {
  if (select.options.skip || select.options.limit) {
    var rownum = connection.client.raw('(dense_rank() over (order by ??)) "row_"',[select.alias + '.' + select.pk]);
    select.selection.push(rownum);
    select.query = connection.client.select('*').from(select.query.as('_SKLMT_PARENT_'));
    if (select.options.skip && select.options.limit) {
      select.query.where('_SKLMT_PARENT_.row_','>', select.options.skip);
      select.query.where('_SKLMT_PARENT_.row_','<=', select.options.limit + select.options.skip);
    } else if (select.options.skip) {
      select.query.where('_SKLMT_PARENT_.row_','>', select.options.skip);
    } else if (select.options.limit) {
      select.query.where('_SKLMT_PARENT_.row_','<=', select.options.limit);
    }
  }
};
...
OracleDialect.prototype.joinSkipLimit = function(connection, select, join) {
    ...
    select.selection.push(rownum);
    select.query.select(select.selection);
    select.selection = [];
    select.query = connection.client.select('*').from(select.query.as(sklmtAlias));
    if (skip && limit) {
      select.query.where(function(){
        this.andWhere(sklmtAlias + '.' + sklmtAlias, '>', skip);
        this.andWhere(sklmtAlias + '.' + sklmtAlias, '<=', skip + limit);        
      });
    } else if (skip) {
      select.query.where(sklmtAlias + '.' + sklmtAlias, '>', skip);
    } else if (limit) {
      select.query.where(sklmtAlias + '.' + sklmtAlias, '<=', limit);
    }
    select.query.orWhereNull(sklmtAlias + '.' + sklmtAlias);
  }
};
