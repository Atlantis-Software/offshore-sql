...
GenericDialect.prototype.select = function(connection, collection, opts) {
  ...
  if (!select.options.select) {
    _.keys(select.definition).forEach(function(field) {
      ...
      if (select.selection.indexOf(column) < 0 && !((select.options.sum || select.options.average || select.options.min || select.options.max) &&
                                                    (select.definition[field].type === 'json' || select.definition[field].type === 'array' || select.definition[field].type === 'text'))) {
  ...
  var aggregate = function(type, columns) {
    columns.forEach(function(key) {
      var alias = self.createAlias('_'+type.toUpperCase()+'_', key);
      select.aggregates.push({tableName: select.alias, columnName: key, alias: alias});
      var tableName = select.alias;
      var subQuery = connection.client(select.tableName + ' as ' + alias)[type](alias + '.' + key + ' as ' + key).as(key);
      subQuery = new self.CriteriaProcessor(connection, alias, select.options, subQuery).getQuery();
      if (select.options.groupBy) {
        select.options.groupBy.forEach(function(groupKey,groupIndex) {
          // subQuery.groupBy(alias + '.' + groupKey);
          select.groupBy[select.alias] = select.groupBy[select.alias] || [];
          select.groupBy[select.alias].push(select.alias + '.' + groupKey);
          delete select.options.groupBy[groupIndex];
          subQuery.andWhereRaw( '?? = ??', [ alias + '.' + groupKey, select.alias + '.' + groupKey] );
        });
      }
  ...
  var aggregateChild = function(type, join, columns) {
    var dataType = 'float';
    columns.forEach(function(key) {
      var alias = self.createAlias(join.alias, key);
      join.select = [];
      select.query[type](join.alias + '.' + key + ' as ' + alias);
      // select.query.groupBy(select.alias + '.' + select.pk);
      select.groupBy[join.child] = select.groupBy[join.child] || [];
      select.groupBy[join.child].push(join.alias + '.' + key);
      select.aggregates.push({tableName: join.alias, columnName: key, alias: alias});
      join.aggregate.push({name: key, alias: alias, type: dataType});
    });
  };
  ...
  if (select.options.joins) {
    select.options.joins.forEach(function(join) {
      ...
      if(join.criteria) {
        if (join.criteria.sum) {
          aggregateChild('sum', join, join.criteria.sum);
        }

        if (join.criteria.average) {
          aggregateChild('avg', join, join.criteria.average);
        }

        if (join.criteria.min) {
          aggregateChild('min', join, join.criteria.min);
        }

        if (join.criteria.max) {
          aggregateChild('max', join, join.criteria.max);
        }
      }
    });
    ...
    _.forEach(relations, function(relation) {
      if (relation.length === 1) {
        self.join(connection, select, relation[0]);
      } else if (relation.length === 2) {
        var join = relation[0];
        var junction = relation[1];
        if(relation[0].select === false) {
          junction = relation[0];
          join = relation[1];
        }
        self.joinManyToMany(connection, select, junction, join);
      }
    });
  }
  ...
  _.forEach(select.groupBy, function(val, key) {
    var item = _.find(select.aggregates, function(item) {
      var res = false;
      val.forEach(function(value) {
        if(item.tableName + '.' + item.columnName === value) {
          res = true;
        }
      });
      return res;
    ...
    if(!item) {
      select.query.groupBy(val);
    }
  });

  select.selection.forEach(function(selection) {
    if (select.aggregates.length) {
      _.forEach(select.groupBy, function(val, key) {
        if (select.groupBy[key].indexOf(selection) === -1) {
          var item = _.find(select.aggregates, function(item) {
            return (item.tableName + '.' + item.columnName === selection || item.alias === selection);
          });
          if(!item && !_.isObject(selection)) {
            select.query.groupBy(selection);
          }
        }
      });
    }
...
GenericDialect.prototype.joinManyToMany = function(connection, select, junction, join) {
  if (join.criteria && (join.criteria.skip || join.criteria.limit)) {
    this.joinManyToManySkipLimit(connection, select, junction, join);
  } else {
    var self = this;
    var childDefinition = connection.getCollection(join.child).definition;
...
GenericDialect.prototype.joinManyToManySkipLimit = function(connection, select, junction, join) {
  var self = this;
  var childDefinition = connection.getCollection(join.child).definition;
  var parent = join.parent;
