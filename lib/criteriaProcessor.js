...
Processor.prototype.processAggregates = function () {
  ...
  if (this.options.sum) {
    this.options.sum.forEach(function (keyToSum) {
      self.query = self.query.sum(self.tableName + '.' + keyToSum + ' as ' + keyToSum);
    });
  }
  if (this.options.average)
    this.options.average.forEach(function (keyToAvg) {
      self.query = self.query.avg(self.tableName + '.' + keyToAvg + ' as ' + keyToAvg);
    });
  if (this.options.min)
    this.options.min.forEach(function (keyToMin) {
      self.query = self.query.min(self.tableName + '.' + keyToMin + ' as ' + keyToMin);
    });
  if (this.options.max)
    this.options.max.forEach(function (keyToMax) {
      self.query = self.query.max(self.tableName + '.' + keyToMax + ' as ' + keyToMax);
    });

  if (this.options.groupBy)
    this.options.groupBy.forEach(function (groupKey) {
      self.query = self.query.groupBy(self.tableName + '.' + groupKey);
    });
  ...
  if (this.options.sort) {
    _.keys(this.options.sort).forEach(function (toSort) {
      ...
      self.query = self.query.orderBy(self.tableName + '.' + toSort, direction);
    });
  }
