...
Processor.prototype.knexify = function(query, col, value, comparator) {
  ...
  if (_.isArray(value)) {
    if (!value.length) {
      return query[this.clause](Knex.raw('1 = 0'));
    }
...
Processor.prototype.processString = function(query, col, comparator, value) {
  var criterion = Knex.raw('LOWER( ??.?? ) ' + comparator + ' ?', [this.tableName, col, value.toLowerCase()]);
...
Processor.prototype.processArray = function(query, col, comparator, value) {
  ...
  inClause += ')';
  return query[this.clause](Knex.raw('??.?? ' + comparator + inClause, [this.tableName, col].concat(value)));
};
