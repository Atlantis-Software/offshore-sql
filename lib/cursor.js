...
var Cursor = function(tableName, connection, joins) {
  ...
  joins.forEach(function(join) {
    if (join.select) {
      self.populated.push(join.alias);
  ...
  dbResults.forEach(function(result) {
    ...
    for (var field in result) {
      if (self.fieldAlias[field]) {
        ...
        var skipFirst = self.fieldAlias[field].skipFirst;
        ...
        if (!index[parentPk]) {
          ...
          _.keys(result).forEach(function(columnName) {
            ...
            if (def) {
              // if model is populated do not add pk value
              if (def.model && self.populated.indexOf(def.alias) >= 0) {
                return;
              }
        ...
        // do not add first child data when populated whith a skip (only parent should be added)
        if (skipFirst && result[skipFirst] === 1) {
          return;
        }
