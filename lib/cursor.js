    ...
    joins.forEach(function (join) {
        if (join.select) {
            ...
            join.select.forEach(function (columnName) {
                ...
                self.fieldAlias[childAlias] = {columnName: columnName, childPk: childPkAlias, parentKey: parentKey, collection: join.collection, childName: join.alias};
            });
        }
    });
};
    ...
    dbResults.forEach(function (result) {
        ...
        for (var field in result) {
            if (self.fieldAlias[field]) {
                ...
                if (childPk) {
                    index[parentPk].children[childName][childPk][childColumnName] = result[field];
                }
